...
namespace CarUsingPolymorphism
{
    public partial class Form1 : MetroFramework.Forms.MetroForm
    {
        ...
        List<Vehicle> filter3 = new List<Vehicle>();
        //**************************FORM 1***********************************
        public Form1()
        {
            InitializeComponent();
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            SortCarList(rand, customerList, vehicleList, pCars, pSportCars, pTrucks);
            //populate comboboxes
            PopulateComboBoxes(vehicleList);
            //empty label
        ...
        private void SortCarList(Random rand, List<int> customers,
            List<Vehicle> cList, List<Car> c, List<SportCar> sc, List<Truck> t)
        {
            //cList is the list that holds all Vehicle objects
            for (int i = 0; i < 30; i++)
            {
                ...
                switch (option)
                {
                    case 1: //case Car list
                        ...
                        else
                        {
                            ...
                            c[index].AddCustomer(customers[cIndex]);
                    ...
                    case 2: //car SportCar list
                        ...
                        else
                        {
                            ...
                            sc[index].AddCustomer(customers[cIndex]);
                    ...
                    case 3: //list pTrucks
                        ...
                        else
                        {
                            ...
                            t[index].AddCustomer(customers[cIndex]);
        ...
        private void PopulateComboBoxes(List<Vehicle> cList)
        {
            foreach (Vehicle c in cList)
            {
        ...
        private void DisplayVehicles(List<Vehicle> vList)
        {
            ...
            foreach (Vehicle v in vList)
            {
                ...
                string[] item = { v.CustomerID.ToString(), v.Make, v.Model, v.Mileage.ToString(),
                                  v.Year.ToString(), v.Price.ToString("c"), v.EngineSize,
                                  v.AverageMpg.ToString(), v.ZeroToSixty.ToString(), v.MaxTowing.ToString(), v.GetType().Name};
        ...
        private Vehicle GetMostExpensiveCar(List<Vehicle> cList)
        {
            //set mostExpensive car as the first in the list
            Vehicle mostExpensive = cList[0];
        ...
        private Vehicle GetLeastExpensiveCar(List<Vehicle> cList)
        {
            //set mostExpensive car as the first in the list
            Vehicle leastExpensive = cList[0];
            //for loop to compare mostExpensive car to all Cars in list
            for (int i = 0; i < cList.Count; i++)
            {
        ...
        private List<Vehicle> GetCarsByMake(string make, List<Vehicle> cList)
        {
            //list of Cars to hold objects filtered by Make
            List<Vehicle> filterMake = new List<Vehicle>();
            for (int i = 0; i < cList.Count; i++)
            {
        ...
        private List<Vehicle> GetCarsByCustomerID(int customerID, List<Vehicle> filter1)
        {
            //list of Cars to hold objects filtered by customerID
            List<Vehicle> filterCustomer = new List<Vehicle>();
        ...
        private List<Vehicle> GetCarsByType(string type, List<Vehicle> filter2)
        {
            //decimal to hold Car object price
            decimal totalPrice = 0;
            //list of Cars to hold objects filtered by Type
            List<Vehicle> filterType = new List<Vehicle>();
        ...
        private void btnMostExpensiveCar_Click(object sender, EventArgs e)
        {
            ...
            //a List<Vehicle> and a parameter
            List<Vehicle> mostExpensiveList = new List<Vehicle>();
            //call method GetMostExpensiveCar and add return to list
            mostExpensiveList.Add(GetMostExpensiveCar(vehicleList));
            //Display most expensive Vehicle
            DisplayVehicles(mostExpensiveList);
        }
        ...
        private void btnLeastExpensive_Click(object sender, EventArgs e)
        {
            ...
            //although it's only one item, the DisplayCars method gets
            //a List<Vehicle> and a parameter
            List<Vehicle> leastExpensiveList = new List<Vehicle>();
            //call method GetLeastExpensiveCar and add return to list
            leastExpensiveList.Add(GetLeastExpensiveCar(vehicleList));
            //Display most expensive Car
            DisplayVehicles(leastExpensiveList);
        }
        ...
        private void btnFilters_Click(object sender, EventArgs e)
        {
            ...
            //filter by Make
            List<Vehicle> filter1 = new List<Vehicle>();
            //filter by CustomerID
            List<Vehicle> filter2 = new List<Vehicle>();
            //conditional statements to call methods according to options
            //in comboboxes
            if (cboMake.Text == String.Empty)
            { filter1 = vehicleList; }
            else
            { filter1 = GetCarsByMake(cboMake.Text, vehicleList); }
